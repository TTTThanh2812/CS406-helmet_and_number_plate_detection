...
ffmpeg_path = r'C:\Users\Thanh\ffmpeg\bin\ffmpeg.exe'
def extract_audio_ffmpeg(video_path, audio_path):
    subprocess.run([ffmpeg_path, '-y', '-i', video_path, '-q:a', '0', '-map', 'a', audio_path])
...
def embed_audio_ffmpeg(video_path, audio_path, output_path):
    ...
    subprocess.run(
        [ffmpeg_path, '-y', '-i', video_path, '-i', audio_path, '-c:v', 'libx264', '-pix_fmt', 'yuv420p', '-c:a', 'aac',
         '-strict', 'experimental', temp_output_path])
